...
addon_id        = 'plugin.video.fireball'
addon           = Addon(addon_id, sys.argv)
AddonTitle          = '[COLOR red]F[/COLOR] [COLOR white]ireball[/COLOR]'
...
PLEXUS_PATH         = xbmc.translatePath('special://home/addons/program.plexus')
baseurl         = 'https://dl.dropbox.com/s/mm3sogophf4ve4p/MainMenu.xml'
...
messagetext     = 'http://mma.areshost6.seedr.io/mma/info.xml'
startinfo       = 'http://mma.areshost6.seedr.io/mma/startinfo.xml'
...
def GetMenu():
   ...
   #xbmc.executebuiltin('Container.SetViewMode(500)')
    url = baseurl
    #addDir('[B][COLOR white]Whats[/COLOR][/B] [B][COLOR red]New[/COLOR][/B]','https://dl.dropbox.com/s/685dwop3wlmtoko/wrzcraftMenu.xml',326,newicon,fanarts)
    ...
    for item in match:
        try:
            ...
            elif '<watchwrestlingup>' in item:
                    name=re.compile('<title>(.+?)</title>').findall(item)[0]
                    iconimage=re.compile('<thumbnail>(.+?)</thumbnail>').findall(item)[0]            
                    fanart=re.compile('<fanart>(.+?)</fanart>').findall(item)[0]
                    url=re.compile('<watchwrestlingup>(.+?)</watchwrestlingup>').findall(item)[0]
                    addDir(name,url,326,iconimage,fanart)
            elif '<watchwrestlingup_menu>' in item:
                    name=re.compile('<title>(.+?)</title>').findall(item)[0]
                    iconimage=re.compile('<thumbnail>(.+?)</thumbnail>').findall(item)[0]            
                    fanart=re.compile('<fanart>(.+?)</fanart>').findall(item)[0]
                    url=re.compile('<watchwrestlingup_menu>(.+?)</watchwrestlingup_menu>').findall(item)[0]
                    addDir(name,url,326,iconimage,fanart)
            #elif '<wrzcraft_menu>' in item:
            #        name=re.compile('<title>(.+?)</title>').findall(item)[0]
            #        iconimage=re.compile('<thumbnail>(.+?)</thumbnail>').findall(item)[0]            
            #        fanart=re.compile('<fanart>(.+?)</fanart>').findall(item)[0]
            #        url=re.compile('<wrzcraft_menu>(.+?)</wrzcraft_menu>').findall(item)[0]
            #        addDir(name,url,324,iconimage,fanart)
            elif '<wrzcraft>' in item:
                    name=re.compile('<title>(.+?)</title>').findall(item)[0]
                    iconimage=re.compile('<thumbnail>(.+?)</thumbnail>').findall(item)[0]            
                    fanart=re.compile('<fanart>(.+?)</fanart>').findall(item)[0]
                    url=re.compile('<wrzcraft>(.+?)</wrzcraft>').findall(item)[0]
                    addDir(name,url,328,iconimage,fanart)
            elif "<reddit>" in item:
                    name=re.compile('<title>(.+?)</title>').findall(item)[0]
...
def DXTV_CATS(url):
    ...
    u = open_url(url)
    match = re.compile('<div class="td-module-thumb"><a href="(.+?)" rel="bookmark" title="(.+?)"><img width="324" height="160" class="entry-thumb" src="(.+?)"',re.DOTALL).findall(u)
...
#    np = re.compile("<a href=\"(.+?)\" >\n.+?<i class=\"td-icon-menu-right\"></i>").findall(u)
#    for nextpage in np:	
...
def SCNRLS_CATS(url):
    ...
    match = re.compile('<h2 class="postTitle"><span></span><a href="(.+?)" title=".+?">(.)(.*?)</a></h2>.+?"https://i.imgaa.com/(.+?)"',re.DOTALL).findall(u)
    for url2,first,rest,img in match:
        image = 'https://i.imgaa.com/' + img
        name = ('[B][COLOR dimgrey]' + first + '[/COLOR][COLOR darkred]' + rest +'[/COLOR][/B]')
...
def WATCHWRESTLINGUP_LINKS(name,url):
    ...
    r = open_url(url).replace('\n', '').replace('\r','').replace('\t','').replace('http://wrestlingtalk.info/1p9u5n3k.php?v=', 'http://www.dailymotion.com/embed/video/').replace('http://sportsnews.pro/cgiv/oload.php?v=','https://oload.stream/embed/')
    r = re.compile('<a class="small red-2 vision-button" href="(.+?)"').findall(r)
    if len(r) == 0: quit()
    elif 'rar' in r:
        pass
    elif len(r) >= 1:
        ...
        for b in r:
            ...
            if urlresolver.HostedMediaFile(b).valid_url():
                a += 1
                streamurl.append(b)
                streamname.append('[B][COLOR red]L[COLOR white]ink %s[/COLOR][/B]' % str(a))
        xbmc.executebuiltin("Dialog.Close(busydialog)")
        if len(streamurl) == 1: PLAYLINK(name,streamurl[0],icon)
        else:
            dialog = xbmcgui.Dialog()
...
def REDDIT_SUGGESTED():
    ...
    r = open_url('http://mma.areshost6.seedr.io/mma/suggested.xml')
...
elif mode==324:WRZCRAFT_CATS(url)
